...
Generative AI tools, specifically OpenAI ChatGPT/Codex, were used for editing
assistance, grammar and style polishing, LaTeX drafting support, and consistency
checking. The measurement code, experimental protocol, numerical results, and
final technical claims were reviewed and approved by the authors. No AI system
was used to generate measurement data.

\section{Artifact Availability}

The anonymized artifact contains four data directories: one batch-1 directory
and one batch-sweep directory for each GPU. Each active directory includes the
trial CSV, summary CSV, environment JSON, and raw power logs. The artifact is
organized so that the reported tables can be recomputed from the submitted
measurement files without relying on external services.
