...
statistics. The artifact also includes a 50-window direct
\texttt{powermetrics} audit for the same five model definitions on a local
Apple M4 Pro MacBook Pro; these repeated-window statistics are reported separately
...
To add auditable trial-level energy evidence, the accompanying repository
includes a direct
...
To make the evidence chain auditable, the repository also includes an expanded
...
logs and the separately provided follow-up audit artifacts are listed
...
\path{MEASUREMENT_PROTOCOL.md}. The repository also includes the follow-up
...
\texttt{powermetrics} audit included with the repository artifact. Each model
has 10 measured 20-second windows, with raw logs and per-window rows stored in
...
collected on the local Apple M4 Pro environment in
Table~\ref{tab:energy-environment}. Absolute values differ from the
paper-aligned mean table because the audit was collected in a later
hardware/software environment. We therefore use it as local repeatability
evidence, not as a replacement for the original source table or as a basis for
retroactively assigning confidence intervals to that table.
...
windows in the released data and summary rather than filtering them, because
...
exact original power-sampling command. The repository adds a follow-up direct
