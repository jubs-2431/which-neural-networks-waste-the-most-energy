...
statistics. Direct validation against independent measurements remains required
future work. The
...
\textbf{(1) An empirical benchmark on Apple Silicon} measuring mean inference
...
report Energy-Delay Product as an energy-latency metric. The central finding
is that latency, not FLOPs, tracks measured energy in this five-model
...
1 Hz; however, the current release does not contain the raw \texttt{powermetrics}
logs, per-window energy traces, the number of measurement windows,
inferences-per-window, window length, or exact original \texttt{powermetrics}
command. We therefore treat the five-model energy measurements as mean
window-attributed energy values only and do not report measured energy standard
...
instantaneous per-inference power resolution. The missing raw energy logs are
listed explicitly in Table~\ref{tab:evidence-status} and in the release
artifact appendix so that future revisions can fill the gap without changing
the reported mean-energy analysis.
...
\begin{tabular}{@{}p{0.32\linewidth}p{0.18\linewidth}p{0.30\linewidth}@{}}
\toprule
Quantity & Status & Repository artifact \\
\midrule
Five-model mean energy and latency & Direct reported means & Apple benchmark CSV \\
Measured energy std., CI, and per-window counts & Not archived & Required future measurement artifact \\
Raw \texttt{powermetrics} logs and exact command & Not archived & Required future measurement artifact \\
17-architecture latency trials & Direct measured trials & Architecture trials CSV \\
17-architecture latency mean/std/p50/p95 & Direct summary & Architecture summary CSV \\
Synthetic energy spread & Supplemental estimate & Power/spread comparison CSV \\
\bottomrule
\end{tabular}
}
\end{table}
...
\texttt{MEASUREMENT\_PROTOCOL.md}. The release does not include raw
\texttt{powermetrics} traces for the original five-model energy table; those
are treated as missing measurement artifacts rather than reconstructed from
synthetic data. Large generated samples and checkpoint files are treated as
derived artifacts and can be regenerated from the repository workflow.
...
preserve per-trial energy spreads, confidence intervals, measurement-window
counts, raw \texttt{powermetrics} logs, or the exact original power-sampling
command. Re-running each model with a controlled number of repeated energy
windows and reporting measured standard deviations and confidence intervals is
the most important extension and would also enable direct validation of the
synthetic spread estimates against held-out measurements.
...
\texttt{measured\_architecture\_trials.csv}. The main five-model energy table
does not have matching raw \texttt{powermetrics} logs in this release. Future
energy releases should add a directory of raw power traces, one integrated
energy row per measurement window, the exact \texttt{powermetrics} command,
window duration, inferences per window, warmup count, cooldown policy, and
...
the exact torchvision version or raw power-sampling command.
