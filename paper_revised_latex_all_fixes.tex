...
A Five-Architecture Benchmark with Exploratory Residualized GAN Modeling}
...
\begin{abstract}
We study per-inference energy for neural network architectures during CPU-only
inference on Apple Silicon, and we develop a paper-aligned synthetic trial
modeling pipeline to support repeated-trial analysis when measurement budgets
are limited. Across five architectures (MobileNetV3-Small, MobileNetV2,
ResNet-18, Tiny-ViT-5M, EfficientNet-B0), we find that, in this five-model
benchmark, latency is strongly associated with measured energy
($R^2 = 0.99999$), while FLOPs explains almost none of the measured energy
variation ($R^2 = 0.00008$). EfficientNet-B0 consumes 2.65$\times$ the
energy of ResNet-18 despite having 4.7$\times$ fewer FLOPs, showing that in
this setup architectural complexity proxies do not capture deployment energy.
We further report Energy-Delay Product (EDP) as a unified
energy-latency metric, under which MobileNetV3-Small has 13.3$\times$ lower
EDP than EfficientNet-B0. This is a per-inference energy/latency comparison,
not an accuracy-normalized efficiency claim. To support exploratory
repeated-trial analysis without re-running the full benchmark, we develop a
residualized conditional WGAN-GP. An initial
...
synthetic component as supplemental: its spreads are support estimates for
repeated trials, not substitutes for independently measured trial-level energy
statistics. This release also includes a 50-window direct
\texttt{powermetrics} audit for the same five model definitions on a local
Apple M1 MacBook Pro; these repeated-window statistics are reported separately
from the original mean table because they were collected in a later local run.
Direct validation against independent measurements remains required future work. The
methodological lesson is that synthetic models of hardware behavior under
sparse data should be tightly scoped and trained on normalized residual
...
evidence available in the released repository is the paper-aligned table
\texttt{paper\_apple\_silicon\_benchmark.csv}, which stores one reported mean
energy and one reported mean latency per model. The source table reports
...
To add auditable trial-level energy evidence, this release includes a direct
follow-up \texttt{powermetrics} audit for the same five model definitions. The
audit records 10 repeated windows per model, 50 windows total, with raw power
...
was collected later on a local Apple M1 MacBook Pro, it is reported as a
reproducibility and variance artifact rather than as the missing raw logs for
the original paper-aligned mean table.

To make the evidence chain auditable, the release also includes an expanded
...
logs and the separately released follow-up audit artifacts are listed
explicitly in Table~\ref{tab:evidence-status} and in the release artifact
appendix so that future revisions can fill the original-source gap without
changing the reported mean-energy analysis.

\begin{table}[ht]
\centering
\caption{Logged hardware, software, and protocol details for the released expanded latency sweep.}
...
Component & Specification \\
\midrule
Device & MacBook Pro (MacBookPro17,1), Apple M1 \\
CPU topology & 8 cores: 4 performance, 4 efficiency \\
Memory & 8 GB RAM \\
Operating system & macOS 26.3, build 25D125 \\
Python & 3.12.1 \\
PyTorch & 2.3.1, CPU backend, float32 \\
torchvision / timm & 0.18.1 / 1.0.24 \\
Threads & \texttt{torch.set\_num\_threads(1)}; inter-op threads = 1 \\
Input and batch & $1 \times 3 \times 224 \times 224$, batch size 1 \\
Power command & \texttt{powermetrics} at 1 Hz; full command in Appendix~\ref{sec:appendix} \\
...
\begin{tabular}{@{}L{0.31\linewidth}L{0.24\linewidth}L{0.32\linewidth}@{}}
\toprule
...
in the accompanying public repository. The key audit files are
\path{paper_apple_silicon_benchmark.csv},
\path{measured_architecture_benchmark.csv},
\path{measured_architecture_trials.csv},
\path{measurement_environment.json}, and
\path{MEASUREMENT_PROTOCOL.md}. The release also includes the follow-up
direct energy audit under \path{measured_energy_powermetrics/}, including
\path{measured_energy_trials.csv},
\path{measured_energy_summary.csv},
\path{measurement_environment_energy.json}, and one raw
\texttt{powermetrics} text log per measured window. The original raw traces
behind the paper-aligned mean table remain unavailable and are not
...
\texttt{powermetrics} audit added in this release. Each model has 10 measured
20-second windows, with raw logs and per-window rows stored in
\texttt{measured\_energy\_powermetrics/}. Confidence intervals are
Student-$t$ 95\% intervals over the 10 measured windows. The audit was
collected on the local Apple M1 environment in
Table~\ref{tab:energy-environment}; it supplies real repeated-window
statistics and raw power traces, but it is not used to retroactively assign
confidence intervals to the original paper-aligned mean table.

One Tiny-ViT-5M window measured 0.7605 J per inference, producing the wide
Tiny-ViT confidence interval in Table~\ref{tab:energy-repeat}. We retain this
window in the released data and summary rather than filtering it, because the
purpose of the audit is to expose measurement variability and make the raw
evidence chain inspectable.

\begin{table*}[ht]
\centering
\caption{Direct repeated \texttt{powermetrics} audit on the local Apple M1 MacBook Pro.}
...
\midrule
MobileNetV3-Small & 10 & 441.0 & 0.0669 & 0.0101 & [0.0597, 0.0741] & 45.39 & 1.473 \\
MobileNetV2 & 10 & 315.0 & 0.0939 & 0.0143 & [0.0837, 0.1041] & 63.62 & 1.475 \\
ResNet-18 & 10 & 262.0 & 0.1264 & 0.0154 & [0.1154, 0.1375] & 76.63 & 1.661 \\
Tiny-ViT-5M & 10 & 134.5 & 0.2759 & 0.1745 & [0.1511, 0.4006] & 149.73 & 1.807 \\
EfficientNet-B0 & 10 & 106.9 & 0.2684 & 0.0276 & [0.2487, 0.2882] & 187.70 & 1.431 \\
\bottomrule
\end{tabular}
\end{table*}
...
\end{axis}
\end{tikzpicture}
\caption{Measured energy versus latency for the five-model benchmark. Linear fit gives $R^2 = 0.99999$.}
...
($R^2 = 0.99999$), while FLOPs explains almost none of the measured energy
variation ($R^2 = 0.00008$). The architectural
...
intervals, measurement-window counts, raw \texttt{powermetrics} logs, or the
exact original power-sampling command. This release adds a follow-up direct
...
a local Apple M1 MacBook Pro, not recovered from the original source run, so
...
rest of the architectures in between. A residualized conditional WGAN-GP,
...
The released repository separates direct measurements, derived proxy columns,
...
\end{verbatim}
The raw per-trial rows for the expanded sweep live in
\texttt{measured\_architecture\_trials.csv}. The follow-up energy audit
...
model order. The original source traces behind
\texttt{paper\_apple\_silicon\_benchmark.csv} remain unavailable.
...
The released GAN workflow fixes the GAN seed at 42 and specifies package
ranges in the repository requirements file: PyTorch $\geq 2.0$, NumPy
...
JSON: Apple M1 MacBook Pro (MacBookPro17,1), 8 GB RAM, macOS 26.3 build
25D125, Python 3.12.1, PyTorch 2.3.1, torchvision 0.18.1, timm 1.0.24, CPU
