...
measured energy nearly perfectly ($R^2 = 0.99999$), while FLOPs explains
almost none of the measured energy variation ($R^2 = 0.00008$). EfficientNet-B0 consumes 2.65$\times$ the
...
rather than validated predictions. The implementation follows the WGAN-GP
training framework \cite{wgangp2017} and tabular conditional generation
motivation from CTGAN-style modeling \cite{ctgan2019}.
...
in the accompanying public repository. Large generated samples and checkpoint
...
energy ($R^2 = 0.99999$), while FLOPs explains only $R^2 = 0.00008$.
...
checks support the qualitative conclusion that latency is the robust predictor
...
\end{axis}
\end{tikzpicture}
\caption{Measured energy versus latency. Linear fit gives $R^2 = 0.99999$.}
...
inference, latency is a near-perfect predictor of energy ($R^2 = 0.99999$),
while FLOPs explains almost none of the measured energy variation ($R^2 = 0.00008$). The architectural
...
Latency predicts energy at $R^2 = 0.99999$, while FLOPs achieves only
$R^2 = 0.00008$. Under Energy-Delay Product, MobileNetV3-Small is 13.4$\times$
...
The released workflow fixes the GAN seed at 42 and specifies package ranges in
the repository requirements file: PyTorch $\geq 2.0$, NumPy $\geq 1.24$, pandas
$\geq 2.0$, scikit-learn $\geq 1.3$, and SciPy $\geq 1.11$. Future measured
benchmark releases should additionally log the exact PyTorch, torchvision,
Python, macOS, chip, and power-sampling versions used during data collection.
