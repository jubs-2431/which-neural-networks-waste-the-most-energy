...
metadata. Across the 17 directly measured architectures, latency explains
...
under this fixed CPU backend. For example, ConvNeXt-micro has only 0.448
...
17-architecture M4 audit, and the retained M5 summary are intentionally kept
...
raw logs and repeated-window uncertainty; and the M5 result is retained as a
cross-device audit summary from the prior HPEC artifact. Agreement between
...
for all 17 architecture variants, but it remains one Apple M4 Pro CPU-only
condition. It does not yet cover additional batch sizes, input shapes,
quantized deployment paths, Metal Performance Shaders, Apple Neural Engine, or
GPU execution. Those conditions require separate matched runs rather than
being mixed into the current CPU table. The \texttt{powermetrics} sampling
interval is also coarse relative to per-inference latency, so energy is
attributed at the window level rather than at the level of one forward pass.

The study is CPU-only on Apple Silicon. It does not evaluate Metal, ANE,
CUDA, or vendor-specific GPU inference backends. Those backends are important,
but they require separate timing synchronization and power logging protocols.
The local rebuild does not include a raw M5 Pro artifact directory, so the M5
entry is reported only as a retained prior HPEC audit summary. Finally, the
residualized WGAN-GP is evaluated against grounded training
